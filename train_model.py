# -*- coding: utf-8 -*-
"""
Created on Sun Jun  11:54:36 2021
@author: Liam
"""
...
def train(epoch):
    ...
    writer.add_graph(module, (torch.randn(1,3,300,300)))
    # module.to(device)
    # 优化器
    ...
    for Epoch in range(epoch):  # 开始训练
        ...
        for ii, (img, label) in enumerate(tqdm(train_dataloader)):
            ...
            writer.add_scalar('loss', loss.data.item(), global_step=ii)
            # print(loss.item())
            loss.backward()  # 反向传播
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', '-ep', help='epoch 训练轮数, 必要参数')
